...
"""
    CuDoubleHT{K,V}(n_buckets::Int; load_factor=DEFAULT_LOAD_FACTOR) -> CuDoubleHT{K,V}
...
table = CuDoubleHT{UInt32,UInt32}(n_buckets)
```
"""
function CuDoubleHT{K,V}(
    n_buckets::Int;
...
"""
    upsert!(status::CuVector{UInt8}, table::CuDoubleHT{K,V}, keys::CuVector{K}, vals::CuVector{V})
...
"""
    upsert!(table::CuDoubleHT{K,V}, keys::CuVector{K}, vals::CuVector{V}) -> CuVector{UInt8}
...
"""
    upsert!(table::CuDoubleHT{K,V}, keys::Vector{K}, vals::Vector{V}) -> Vector{UInt8}
